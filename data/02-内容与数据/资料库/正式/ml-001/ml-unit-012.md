---
content_unit_id: ml-unit-012
book_id: ml-001
topic_id: rl-001
chapter: 强化学习基础
knowledge_points: [math-prob, rl-discount, algo-dp, rl-bellman, rl-mdp, rl-qlearning]
source_id: src-ocademy-ml
source_relative_path: open-machine-learning-jupyter-book/deep-learning/dqn.ipynb
source_commit: b1a5645749b1e3ec9977693b01c662014b71cae5
license: CC-BY-4.0
source_url: https://press.ocademy.cc/deep-learning/dqn.html
...
完成本节后，你应能解释并应用：概率与长期回报、折扣因子、动态规划、贝尔曼方程、MDP、Q-Learning。

## 阅读重点

理解奖励累积与期望回报；理解并计算未来奖励折扣；理解递推与最优子结构；理解价值函数递推关系；理解状态动作转移奖励框架；理解无模型动作价值更新
...
```python
class NeuralNetwork:
    def __init__(self, input_size, num_memory_units):
        self.model = self.create_model(input_size, num_memory_units)
        
    def create_model(self, input_size, num_memory_units):
        model = tf.keras.models.Sequential([
            tf.keras.layers.Dense(6, activation='sigmoid', input_shape=(input_size,)),
            tf.keras.layers.Dense(6, activation='sigmoid'),
            tf.keras.layers.Dense(4 + num_memory_units)
        ])
        return model
...
Assignment - DQN on foreign exchange market（请通过本页“官方章节”链接查看）
