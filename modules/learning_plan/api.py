...
from .schemas import (
    ...
    LearningPlanLookupResponse,
    MaterialLearningPlanRequest,
)
...
def build_router(
    ...
    @router.post(
        "/api/learning-plans/generate", response_model=GenerateLearningPlanResponse
    )
    ...
    @router.post(
        "/api/learning-plans/material", response_model=GenerateLearningPlanResponse
    )
    def create_material_learning_plan(
        payload: MaterialLearningPlanRequest,
        current_user: CurrentUser = current_user_dependency,
    ) -> dict[str, Any]:
