...
from modules.common.time import business_now, business_today
...
class TodayLearningModule:
    ...
    def get_today_learning(self, *, user_id: str, book_id: str) -> dict[str, Any]:
        ...
        book = BOOKS.get(
            ...
            {
                "id": values["book_id"],
                "title": values["book_id"],
                "shortTitle": values["book_id"],
                "subtitle": "",
            },
        )
        plan = self.learning_plan.get_saved(
            book_id=values["book_id"],
            user_id=values["user_id"],
        )
        activities = [
            activity
            for activity in self.learning_record.list_activities(
                values["user_id"], page=1, page_size=100
            )["records"]
            if activity.book_id
            in {
                values["book_id"],
                RECORD_BOOK_IDS.get(values["book_id"], values["book_id"]),
            }
        ]
        ...
        payload = {
            ...
            "weeklyProgress": {
                ...
                "studyDurationHours": round(
                    result.weekly_progress.study_duration_seconds / 3600, 2
                ),
    ...
    def _tasks(plan: dict[str, Any] | None) -> list[dict[str, Any]]:
        ...
        for task in plan.get("tasks") or []:
            expected_date = task.get("expected_completion_date") or task.get(
    ...
    def _knowledge_graph(
        activities: list[LearningActivity], goal: str, tasks: list[dict[str, Any]]
    ) -> dict[str, Any]:
        latest = next(
            (
                item
                ...
                and item.result.get("knowledge_point_results")
            ),
            None,
        )
        task_by_knowledge_point = {
            point_id: task
            for task in tasks
            for point_id in task.get(
                "knowledge_point_ids", task.get("knowledgePointIds", [])
            )
        }
        nodes = []
        for item in latest.result.get("knowledge_point_results", []) if latest else []:
            ...
            status = (
                "weak"
                if raw_status in {"不会", "基本了解"}
                else "good"
                if raw_status == "掌握"
                else "learning"
            )
            knowledge_point_id = item.get("knowledge_point_id", "")
            task = task_by_knowledge_point.get(knowledge_point_id) or next(
                (
                    candidate
                    for candidate in tasks
                    if str(candidate.get("id", "")).endswith(f"-{knowledge_point_id}")
                ),
                {},
            )
            nodes.append(
                {
                    ...
                    "label": KNOWLEDGE_POINT_LABELS.get(
                        knowledge_point_id, knowledge_point_id
                    ),
                    "status": status,
                    "accuracy": round(score * 100, 2) if score is not None else None,
                    "masteryScore": score,
                    "taskId": task.get("id"),
    ...
    def _weekly_progress(
        activities: list[LearningActivity], tasks: list[dict[str, Any]]
    ) -> WeeklyProgress:
        now = business_now()
        ...
        for activity in activities:
            try:
                occurred = datetime.fromisoformat(
                    activity.occurred_at.replace("Z", "+00:00")
                )
            except ValueError:
                continue
            if occurred.tzinfo is None:
                occurred = occurred.replace(tzinfo=timezone.utc)
            occurred = occurred.astimezone(now.tzinfo)
            if occurred < start:
                continue
            weekly.append(activity)
            seconds = int(
                activity.result.get(
                    "duration_seconds", activity.result.get("durationSeconds", 0)
                )
                or 0
            )
            ...
            correct += int(
                activity.result.get(
                    "correct_count", activity.result.get("correctCount", 0)
                )
                or 0
            )
            total += int(
                activity.result.get("total_count", activity.result.get("totalCount", 0))
                or 0
            )
        completed = sum(task.get("status") == "completed" for task in tasks)
        total_tasks = len(tasks)
        return WeeklyProgress(
            progress_percent=round(completed / total_tasks * 100, 2)
            if total_tasks
            else 0,
            ...
            daily_duration=[
                {"date": day, "durationSeconds": seconds}
    ...
    @staticmethod
    def _recommendation(
        tasks: list[dict[str, Any]], graph: dict[str, Any]
    ) -> dict[str, Any] | None:
    ...
    def _continue_learning(tasks: list[dict[str, Any]]) -> dict[str, Any] | None:
        task = next(
            (item for item in tasks if item.get("status") == "in_progress"), None
        )
        task = task or next(
            (item for item in tasks if item.get("status") == "todo"), None
        )
        if not task:
            return None
        return {
            ...
            "expectedCompletionDate": task.get("expected_completion_date")
            or task.get("expectedCompletionDate", ""),
            "description": task.get("description", ""),
            "reason": task.get("reason", ""),
        }
