...
function App() {
  ...
  const currentQuestion = content.questions[diagnosticIndex] ?? content.questions[0];
  ...
  const startDiagnostic = async () => {
    ...
    try {
      const result = await api.startDiagnostic(bookId);
  ...
  const advanceDiagnostic = async (question: DiagnosticQuestion, answer?: string, skipped = false) => {
    ...
    try {
      ...
      if (diagnosticIndex >= content.questions.length - 1) {
